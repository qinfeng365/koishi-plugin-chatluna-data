...
var root = typeof __dirname === "string" ? __dirname : (0, import_path.dirname)((0, import_url.fileURLToPath)(import_meta.url));
...
var Config = import_koishi.Schema.object({
  pageSize: import_koishi.Schema.number().min(10).max(200).default(40).description("\u9ED8\u8BA4\u5206\u9875\u5927\u5C0F\u3002"),
  readonly: import_koishi.Schema.boolean().default(false).description("\u53EA\u8BFB\u6A21\u5F0F\u4F1A\u9690\u85CF\u5199\u6309\u94AE\uFF0C\u5E76\u62D2\u7EDD\u6240\u6709\u5199 RPC\u3002"),
  maxPreviewRows: import_koishi.Schema.number().min(10).max(1e3).default(200).description("\u5371\u9669\u64CD\u4F5C\u9884\u89C8\u6700\u591A\u8FD4\u56DE\u7684\u5BF9\u8C61\u6570\u91CF\u3002"),
  enableArchiveFileOps: import_koishi.Schema.boolean().default(false).description("\u5141\u8BB8\u68C0\u67E5\u5F52\u6863\u6587\u4EF6\u662F\u5426\u5B58\u5728\u4E0E\u8BFB\u53D6\u6587\u4EF6\u5927\u5C0F\u3002"),
  enableMessageRepair: import_koishi.Schema.boolean().default(false).description("\u5141\u8BB8\u6267\u884C\u6D88\u606F\u94FE parentId/latestMessageId \u4FEE\u590D\u3002"),
  identityRefreshLimitPerBatch: import_koishi.Schema.number().min(1).max(200).default(30).description("\u6BCF\u6B21\u6279\u91CF\u5237\u65B0\u5E73\u53F0\u8EAB\u4EFD\u8D44\u6599\u7684\u6700\u5927\u6570\u91CF\u3002"),
  identityRefreshInterval: import_koishi.Schema.number().min(0).max(1e4).default(800).role("ms").description("\u6279\u91CF\u5237\u65B0\u5E73\u53F0\u8EAB\u4EFD\u8D44\u6599\u65F6\uFF0C\u6BCF\u4E2A\u8BF7\u6C42\u4E4B\u95F4\u7684\u7B49\u5F85\u65F6\u95F4\u3002"),
  inactiveWarningDays: import_koishi.Schema.number().min(1).max(3650).default(180).description("\u8D85\u8FC7\u591A\u5C11\u5929\u672A\u6D3B\u8DC3\u65F6\u5728\u6743\u9650\u9875\u63D0\u793A\u3002"),
  enableIdentityLookup: import_koishi.Schema.boolean().default(true).description("\u5141\u8BB8\u624B\u52A8\u8C03\u7528\u5E73\u53F0 API \u5237\u65B0\u7528\u6237\u6635\u79F0\u548C\u5934\u50CF\u3002"),
  enableKoishiPermissionCommands: import_koishi.Schema.boolean().default(true).description("\u6CE8\u518C Koishi \u6743\u9650\u6CBB\u7406\u6307\u4EE4\u3002")
});
var audits = [];
var opsErrors = [];
var rootCtx;
...
function apply(ctx, cfg) {
  ...
  ctx.console.addEntry({
    dev: (0, import_path.resolve)(root, "../client/index.ts"),
    prod: (0, import_path.resolve)(root, "../dist")
  });
  const opsTarget = {
    levels: { base: import_koishi.Logger.WARN },
    record: (record) => {
      void recordOpsLog(ctx, cfg, record).catch(() => {
      });
    }
  };
  import_koishi.Logger.targets.push(opsTarget);
  ctx.on("dispose", () => {
    const idx = import_koishi.Logger.targets.indexOf(opsTarget);
    if (idx >= 0) import_koishi.Logger.targets.splice(idx, 1);
  });
...
${row.bindingKeys.join("\n")}`,
      ...
      if (fastPath) {
        const [pageMsgs, total, convs2] = await Promise.all([
          ...
          ctx.database.select("chatluna_message", msgQuery).execute(
            (row) => import_koishi.$.count(row.id)
          ),
          getRows(ctx, "chatluna_conversation")
        ]);
  ...
  ctx.console.addListener(
    ...
    async (input) => {
      ...
      const now = /* @__PURE__ */ new Date();
      const row = {
        id: input.row.id || (0, import_crypto.randomUUID)(),
...
async function viewArchive(row, cfg) {
  ...
  try {
    await (0, import_promises.access)(row.path, import_fs.constants.R_OK);
    return {
      ...row,
      createdAt: iso(row.createdAt),
      restoredAt: iso(row.restoredAt),
      fileState: "ok",
      fileSize: (await (0, import_promises.stat)(row.path)).size
    };
  } catch {
    return {
      ...row,
