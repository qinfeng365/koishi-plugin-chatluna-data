...
${row.permission}`,
  ...
  ctx.console.addListener(
    ...
    async () => {
      const [users, kBindings, channels, convs, msgs, acls, rules] = await Promise.all([
        getRows(ctx, "user"),
        getRows(ctx, "binding"),
        getRows(ctx, "channel"),
        getRows(ctx, "chatluna_conversation"),
      ...
      const nameHints = /* @__PURE__ */ new Map();
      ...
      const userRows = users.map((row) => {
        ...
        const display = row.name || ids.map((id) => nameHints.get(id)).find(Boolean) || ids[0] || `\u7528\u6237 ${row.id}`;
        const last = ids.map((id) => lastActiveByPrincipal.get(id)).filter(Boolean).sort((a, b) => timeOf(b) - timeOf(a))[0];
        return {
          id: row.id,
          name: row.name,
          displayName: display,
          nameSource: row.name ? "koishi" : ids.some((id) => nameHints.has(id)) ? "chatluna" : ids.length > 0 ? "binding" : "fallback",
          authority: row.authority,
          permissions: row.permissions ?? [],
          createdAt: iso(row.createdAt),
          lastActiveAt: iso(last),
          inactiveDays: last ? Math.floor((Date.now() - timeOf(last)) / 864e5) : null,
      ...
      const issues = [
        ...
        ...userRows.filter(
          (row) => row.inactiveDays != null && row.inactiveDays >= 180 && row.authority > 0
        ).map((row) => ({
          level: "warning",
          type: "inactive-user",
          target: row.displayName,
