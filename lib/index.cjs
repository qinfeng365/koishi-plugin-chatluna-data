  ...
  ctx.console.addListener(
    ...
    async () => {
      ...
      const userRows = users.map((row) => {
        ...
        const rows = convs.filter(
          (conv) => ids.includes(conv.createdBy) || ids.some((id) => conv.bindingKey.includes(id))
        );
        return {
          id: row.id,
          name: row.name,
          ...
          acl: acls.filter((acl) => ids.includes(acl.principalId)).length,
          constraints: rules.filter(
            (rule) => ids.some(
              (id) => [
      ...
      const issues = [
        ...
        ...kBindings.filter((row) => !users.some((user) => user.id === row.aid)).map((row) => ({
          level: "warning",
      ...
      return {
        ...
        channels: channels.map((row) => ({
          ...
          permissions: row.permissions ?? [],
          createdAt: iso(row.createdAt),
          conversations: convs.filter(
            (conv) => conv.bindingKey.includes(row.id)
          ).length,
          acl: acls.filter(
            (acl) => acl.principalType === "guild" && acl.principalId === row.id
          ).length
        })).sort(
